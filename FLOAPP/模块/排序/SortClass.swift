...
/// 排序类的简单工厂
class SortFactory {
    static func create(_ type: SortTypeEnum) -> SortType {
        switch type {
        case .BubbleSort:
            return BubbleSort()
...
class SortBaseClass {
    ...
    func displayResult(_ index: Int, value: Int) {
    ...
    func successSort(_ sortList:Array<Int>) -> Void {
        ...
        for i in 0..<list.count {
            ...
            while j > i {
                if list[j - 1] > list[j]  { //前边的大于后边的则进行交换
                    ...
                    displayResult(j-1, value: list[j-1])
            ...
            for i in 0..<list.count {
                ...
                while j >= step && j < list.count {
                    if list[j] < list[j - step]  {
                        ...
                        displayResult(j, value: list[j])
                        displayResult(j-step, value: list[j-step])
    ...
    func sort(_ items: Array<Int>) -> Array<Int> {
        ...
        for i in 0..<list.count {
            ...
            if minIndex != i {
                ...
                displayResult(i, value: list[i])
                displayResult(minIndex, value: list[minIndex])
            }
            //print("本轮结果为：\(list)\n")
        }
        self.successSort(list)
...
class HeapSort: SortBaseClass, SortType {
    ...
    func sort(_ items: Array<Int>) -> Array<Int> {
        ...
        while endIndex >= 0 {
            ...
            displayResult(0, value: list[0])
            displayResult(endIndex, value: list[endIndex])
    ...
    func heapCreate(_ items: inout Array<Int>) {
        var i = items.count
        while i > 0 {
    ...
    func heapAdjast(_ items: inout Array<Int>, startIndex: Int, endIndex: Int) {
        ...
        while maxChildIndex <= endIndex {
            ...
            if temp < items[maxChildIndex-1] {
                ...
                displayResult(fatherIndex-1, value: items[fatherIndex-1])
            } else {
                break
            }
...
class MergingSort: SortBaseClass, SortType {
    ...
    func sort(_ items: Array<Int>) -> Array<Int> {
        ...
        while tempArray.count != 1 {
            ...
            while i < tempArray.count - 1 {
                ...
                tempArray[i] = mergeArray(tempArray[i], secondList: tempArray[i + 1])
                tempArray.remove(at: i + 1)
                for subIndex in 0..<tempArray[i].count{
                   let index = self.countSubItemIndex(i, subItemIndex: subIndex)
                   self.displayResult(index, value: tempArray[i][subIndex])
                    
                }
                i = i + 1
            }
        }
        self.successSort(tempArray.first!)
    ...
    ///
    /// - returns: 返回排序好的数组
    func mergeArray(_ firstList: Array<Int>, secondList: Array<Int>) -> Array<Int> {
    ...
    func countSubItemIndex(_ endIndex: Int, subItemIndex: Int) -> Int {
        var sum = 0
        for i in 0..<endIndex {
...
class QuickSort: SortBaseClass, SortType {
    ...
    fileprivate func quickSort(_ list: inout Array<Int>, low: Int, high: Int) {
        if low < high {
            let mid = partition(&list, low: low, high: high)
            quickSort(&list, low: low, high: mid - 1)   //递归前半部分
            quickSort(&list, low: mid + 1, high: high)  //递归后半部分
        }
    }
    ...
    fileprivate func partition(_ list: inout Array<Int>, low: Int, high: Int) -> Int {
        var low = low
        var high = high
        let temp = list[low]
        ...
        while low < high {
            ...
            list[low] = list[high]
            displayResult(low, value: list[low])
            
            while low < high && list[low] <= temp {
                low += 1
            }
            list[high] = list[low]
            displayResult(high, value: list[high])
        }
        list[low] = temp
        displayResult(low, value: list[low])
...
class RadixSort: SortBaseClass, SortType {
    ...
    fileprivate func radixSort(_ list: inout Array<Int>) {
        var bucket = createBucket()
        let maxNumber = listMaxItem(list)
        let maxLength = numberLength(maxNumber)
        
        for digit in 1...maxLength {
            //入桶
            for item in list {
                let baseNumber = fetchBaseNumber(item, digit: digit)
            ...
            for i in 0..<bucket.count {
                while !bucket[i].isEmpty {
                    ...
                    displayResult(index, value: list[index])
                    index += 1
                }
            }
        }
    ...
    ///
    /// - returns: 返回创建好的桶子
    fileprivate func createBucket() -> Array<Array<Int>> {
    ...
    ///
    /// - returns: 返回该数列中最大的值
    fileprivate func listMaxItem(_ list: Array<Int>) -> Int {
    ...
    ///
    /// - returns: 返回该位数上的数字
    func fetchBaseNumber(_ number: Int, digit: Int) -> Int{
