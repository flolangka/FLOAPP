...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func tapSegmentContol(_ sender: UISegmentedControl) {
        ...
        for i in 0..<self.sortViews.count {
            self.updateSortViewHeight(i, value: CGFloat(sortViewHight[i]))
        }
        
        let sortType = SortTypeEnum(rawValue: sender.selectedSegmentIndex)!
        self.sort = SortFactory.create(sortType)
        self.setSortClosure()
    }
    ...
    @IBAction func tapSortButton(_ sender: AnyObject) {
        ...
        DispatchQueue.global().async {
            self.sortViewHight = self.sort.sort(self.sortViewHight)
        }
    }
    ...
    //MARK: - Private Method
    /// 设置排序对象相关的回调
    fileprivate func setSortClosure() {
        weak var weak_self = self
        sort.setEveryStepClosure({ (index, value) in
            DispatchQueue.main.async {
                weak_self?.updateSortViewHeight(index, value: CGFloat(value))
            }
        }) { (list) in
            DispatchQueue.main.async {
    ...
    /// 随机生成sortView的高度
    fileprivate func configSortViewHeight() {
    ...
    fileprivate func updateSortViewHeight(_ index: Int, value: CGFloat) {
        self.sortViews[index].updateHeight(value)
    }
